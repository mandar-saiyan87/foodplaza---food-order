...
export const getordersCurrentUser = createAsyncThunk(
  "getordersCurrentUser",
  async (orders) => {
    // console.log(orders);
    const { userid, currentPage } = orders;
    const req = await fetch(
      `${process.env.REACT_APP_API_URL}/api/orders/currentuser/${userid}?page=${currentPage}`
    );
    const data = req.json();
    // console.log(data);
    return data;
...
export const cartSlice = createSlice({
  ...
  reducers: {
    addtocart: (state, action) => {
      ...
      const menuExist = state.cartItems.findIndex(
        (menu) => menu.menuItem._id === menuItem._id
      );
      if (menuExist !== -1) {
  ...
  extraReducers: (builder) => {
    builder
      .addCase(sendtocart.pending, (state) => {
        state.loading = true;
      })
    ...
    builder
      ...
      .addCase(updateOrders.fulfilled, (state, action) => {
        ...
        const updatedOrder = state.orders.findIndex(
    ...
    builder
      .addCase(getordersCurrentUser.pending, (state) => {
        state.loading = true;
      })
...
export const cartSubtotal = (state) =>
  state.cart.cartItems.reduce(
    (price, item) => price + item.menuItem.price * item.qty,
    0
  );
...
export const deliveryFees = createSelector(
  ...
  cartSubtotal,
  (cart, subtotal) =>
    (subtotal < cart.freeDelivery) & (subtotal !== 0) ? cart.delivery : 0
);
